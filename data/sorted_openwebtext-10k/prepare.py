...
if __name__ == '__main__':
    ...
    freqDict = dict()
    lineCount = 1
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)),"output.txt")) as freqFile:
        wordFreq = freqFile.readlines();
    for line in wordFreq:
        lowercase = line
        lowercase = lowercase.lstrip()
        lowercase = lowercase.rstrip()
        lowercase = lowercase.rstrip('.')
        if lowercase not in freqDict:
            freqDict[lowercase] = lineCount
        lineCount += 1

    # Function to calculate the length of the text
    def calculate_text_difficulty(example):
        ps = PorterStemmer()
        lineScore = 0
        wordCount = 0
        strSplit = example['text'].split()
        for word in strSplit:
            word = re.sub('[\W_]+', '', word)
            lowerWord = word.lower()
            ps.stem(lowerWord)
            if lowerWord in freqDict:
                lineScore += freqDict[lowerWord]
            else:
                lineScore += 5050
            wordCount += 1
        return lineScore/wordCount
    ...
    sorted_train_dataset = sorted(split_dataset['train'], key=calculate_text_difficulty)
    sorted_val_dataset = sorted(split_dataset['val'], key=calculate_text_difficulty)
    ...
    filename_val = 'openwebtext-10k_sorted_val_dataset.bin'
    dtype_val = np.uint16
